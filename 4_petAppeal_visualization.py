...
print 'There are ', len(breed), 'unique breeds'
...
adopted = shelter_animals[(shelter_animals.status == 'X')]
available = shelter_animals[(shelter_animals.status == 'A')]
...
for key, value in reorder_dict.iteritems():
    ...
    df.columns = status
    df = df.reset_index()
    petAppeal.group_bar_graph(df, ['X', 'A'], key)
